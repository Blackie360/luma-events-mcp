...
test("event deletion previews impact without canceling", async () => {
  ...
  const preview = await deleteEvent(
    { event_id: "evt-test", confirmed: false },
...
test("event deletion uses Luma's two-step cancellation flow", async () => {
  ...
  const request = async (path: string, options: { body?: unknown } = {}) => {
    requests.push({ path, body: options.body });
    if (path === "/v1/events/get") return { name: "Paid Event" };
    if (path === "/v1/events/cancel/request") {
  ...
  const deleted = await deleteEvent(
    { event_id: "evt-paid", should_refund: true, confirmed: true },
    request as LumaRequest
  );
...
test("event deletion requires an explicit refund choice for paid events", async () => {
  ...
  await assert.rejects(
    deleteEvent({ event_id: "evt-paid", confirmed: true }, request as LumaRequest),
...
test("waitlist approval stays below the conservative write limit and resumes safely", async () => {
  ...
  const first = await approveWaitlistedGuests("evt-test", { send_email: false }, request as LumaRequest);
  ...
  const second = await approveWaitlistedGuests("evt-test", { send_email: false }, request as LumaRequest);
...
test("sendInvites batches recipients and returns no identities", async () => {
  ...
  const request = async (path: string, options: { body?: unknown } = {}) => {
    ...
    const body = options.body as { event_id: string; guests: unknown[]; message: string };
  ...
  const summary = await sendInvites("evt-target", guests, "Join us", request as LumaRequest);
...
test("inviteGuestsFromEvent previews aggregate deduplication without writing or exposing identities", async () => {
  let writes = 0;
  const request = async (path: string, options: { query?: Record<string, unknown> } = {}) => {
...
test("inviteGuestsFromEvent sends the confirmed audience in resumable batches", async () => {
  const inviteBatches: Array<Array<{ email: string }>> = [];
  const request = async (path: string, options: { query?: Record<string, unknown>; body?: unknown } = {}) => {
    if (path === "/v1/events/guests/list") {
      if (options.query?.event_id === "evt-target") return { entries: [], has_more: false };
      return {
    ...
    assert.equal(path, "/v1/events/guests/send-invites");
    const body = options.body as { event_id: string; guests: Array<{ email: string }>; message: string };
    assert.equal(body.event_id, "evt-target");
...
test("inviteGuestsFromEvent rejects a source event used as its own target", async () => {
  ...
  await assert.rejects(
    inviteGuestsFromEvent({ source_event_id: "evt-same", target_event_id: "evt-same" }, request as LumaRequest),
    /must be different/
  );
  assert.equal(called, false);
});
