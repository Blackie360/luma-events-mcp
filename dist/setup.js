...
export function credentialsPath(env = process.env, home = homedir(), platform = process.platform) {
    if (env.LUMA_API_KEY_FILE?.trim())
        return env.LUMA_API_KEY_FILE.trim();
...
export function readStoredApiKey(env = process.env, home = homedir(), platform = process.platform) {
    try {
        const parsed = JSON.parse(readFileSync(credentialsPath(env, home, platform), "utf8"));
...
export async function storeApiKey(apiKey, path = credentialsPath()) {
...
async function executablePath(names, env = process.env, platform = process.platform) {
...
export function cursorConfigPath(env = process.env, home = homedir()) {
    return env.CURSOR_MCP_CONFIG?.trim() || join(home, ".cursor", "mcp.json");
}
export async function detectClients(env = process.env, home = homedir(), platform = process.platform) {
    const detected = [];
...
export function parseClientSelection(value, detected) {
...
export async function verifyLumaApiKey(apiKey, apiBase = process.env.LUMA_API_BASE?.trim() || DEFAULT_API_BASE, fetcher = fetch) {
...
export async function runCommand(command, args, timeoutMs = 45_000) {
...
function commandForClient(client) {
...
async function installCursor(path = cursorConfigPath()) {
    let existing = {};
    let backup;
...
export async function installClient(client, runner = runCommand, cursorPath = cursorConfigPath()) {
    if (client.id === "cursor")
        return await installCursor(cursorPath);
...
export class InteractivePrompter {
    ...
    async ask(question, secret = false) {
        if (this.lineIterator) {
            ...
            if (next.done)
                throw new Error("Input ended before setup was complete.");
            if (secret)
                this.output.write("\n");
            return next.value.trim();
        }
        if (secret && this.input.setRawMode)
            return await this.askSecret(question);
        return await new Promise((resolve) => {
