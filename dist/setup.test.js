...
import { credentialsPath, detectClients, installClient, parseClientSelection, readStoredApiKey, runInteractiveSetup, storeApiKey, verifyLumaApiKey } from "./setup.js";
...
test("credentials are stored once with owner-only permissions and can be read by the server", async () => {
    ...
    assert.equal(readStoredApiKey({ LUMA_API_KEY_FILE: path }, directory, process.platform), "secret-calendar-key");
...
test("credentialsPath supports explicit, XDG, and Windows locations", () => {
    assert.equal(credentialsPath({ LUMA_API_KEY_FILE: "/secure/luma.json" }, "/home/test", "linux"), "/secure/luma.json");
    assert.equal(credentialsPath({ XDG_CONFIG_HOME: "/xdg" }, "/home/test", "linux"), join("/xdg", "luma-events-mcp", "credentials.json"));
    assert.equal(credentialsPath({ APPDATA: "C:\\Users\\test\\AppData\\Roaming" }, "C:\\Users\\test", "win32"), join("C:\\Users\\test\\AppData\\Roaming", "luma-events-mcp", "credentials.json"));
});
...
test("selection accepts numbers, client names, all, and removes duplicates", () => {
    ...
    assert.deepEqual(parseClientSelection("1, gemini, 1", detected).map((client) => client.id), ["codex", "gemini"]);
...
test("Luma key verification sends the secret only in the API header", async () => {
    let requestedUrl = "";
    let requestedHeader = "";
    await verifyLumaApiKey("private-key", "https://luma.test", async (input, init) => {
    ...
    await assert.rejects(verifyLumaApiKey("bad-key", "https://luma.test", async () => new Response("", { status: 401 })), /HTTP 401/);
});
...
test("Cursor installation preserves other settings and creates a backup", async () => {
    ...
    const result = await installClient({ id: "cursor", label: "Cursor", detection: "test" }, async () => ({ code: 0, stdout: "", stderr: "" }), path);
